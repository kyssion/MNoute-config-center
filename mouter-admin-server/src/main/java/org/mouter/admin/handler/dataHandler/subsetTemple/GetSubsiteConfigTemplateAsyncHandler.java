...
public class GetSubsiteConfigTemplateAsyncHandler extends AsyncSampleFnHandler {
    ...
    public GetSubsiteConfigTemplateAsyncHandler(String name) {
        super(name);
    }
    ...
    public void asyncHandle(ParamWrapper paramWrapper, AsyncResult asyncResult, AsyncScheduler asyncScheduler) {
        MysqlPool.mysql.getConnection((res)-> {
            if (res.succeeded()) {
                ...
                connection.preparedQuery("select * from config_subset_template where group_id=? and app_id=? and subset_id=? limit ?,?")
                        .execute(Tuple.of(appdata.getGroupId(), appdata.getAppId(), appdata.getSudsetId(),appdata.getPage().getPage(), appdata.getPage().getSize()), (result) -> {
                            if (result.succeeded()) {
                                connection.preparedQuery("select count(*) as num from config_subset_template where group_id=? and app_id=? and subset_id=? limit ?,?")
                                        .execute(Tuple.of(appdata.getGroupId(), appdata.getAppId(), appdata.getPage().getPage(), appdata.getPage().getSize()), (r -> {
                                            if (r.succeeded()) {
                                                List<SudsetConfigTemplateData> applicationInformationData = ObjectUtils.getDataFrom(result.result(), SudsetConfigTemplateData.class);
