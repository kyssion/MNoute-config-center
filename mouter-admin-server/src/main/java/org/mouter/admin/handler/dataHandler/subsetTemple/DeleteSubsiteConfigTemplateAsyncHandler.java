...
@MintFlowHandler
public class DeleteSubsiteConfigTemplateAsyncHandler extends AsyncSampleFnHandler {

    public static String SQL_DATA_KEY = "data.delete.subsiteConfigTemplate";
    public static String SQL_DATA_RESULT_KEY = "data.delete.application.subsiteConfigTemplate";

    public DeleteSubsiteConfigTemplateAsyncHandler(String name) {
        super(name);
    }
    ...
    public void asyncHandle(ParamWrapper paramWrapper, AsyncResult asyncResult, AsyncScheduler asyncScheduler) {
        MysqlPool.mysql.getConnection((res)->{
            if(res.succeeded()){
                ...
                if(ObjectUtils.isNullOrEmpty(appData.getAppId(),appData.getGroupId(),appData.getSubsetName())){
