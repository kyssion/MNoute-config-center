        ...
        MysqlPool.mysql.getConnection((res)->{
            if(res.succeeded()){
                ...
                if(ObjectUtils.isNullOrEmpty(appdata.getAppId())){
                    connection.preparedQuery("select * from config_app_data where group_id=? and app_id=? limit ?,?")
                            .execute(Tuple.of(appdata.getGroupId(),appdata.getAppId(),appdata.getPage().getPage(),appdata.getPage().getSize()),(result)->{
                                if(result.succeeded()){
                                    connection.preparedQuery("select count(*) as num from config_app_data where group_id=? and app_id=? limit ?,?")
                                            .execute(Tuple.of(appdata.getGroupId(),appdata.getAppId(),appdata.getPage().getPage(),appdata.getPage().getSize()),(r->{
                                                if(r.succeeded()){
                                                    List<ConfigAppData> applicationInformationData = ObjectUtils.getDataFrom(result.result(),ConfigAppData.class);
                    ...
                    connection.preparedQuery("select * from config_app_data where group_id=? and app_id=? and app_data_id=? limit ?,?")
                            .execute(Tuple.of(appdata.getGroupId(),appdata.getAppId(),appdata.getAppDataId(),appdata.getPage().getPage(),appdata.getPage().getSize()),(result)->{
                                if(result.succeeded()){
                                    connection.preparedQuery("select * from config_app_data where group_id=? and app_id=? and app_data_id=? limit ?,?").execute(
                                            Tuple.of(appdata.getGroupId(),appdata.getAppId(),appdata.getAppDataId(),appdata.getPage().getPage(),appdata.getPage().getSize()),r->{
                                                if(r.succeeded()){
                                                    List<ConfigAppData> applicationInformationData = ObjectUtils.getDataFrom(result.result(),ConfigAppData.class);
                                                    RowSet<Row> rows = r.result();
                                                    Integer integer = rows.iterator().next().getInteger("num");
