...
public class InsertSubsiteConfigTemplateStructureAsyncHandler extends AsyncSampleFnHandler {

    public static String SQL_DATA_KEY = "data.insert.config";
    public static String SQL_DATA_RESULT_KEY = "data.insert.config.result";


    public InsertSubsiteConfigTemplateStructureAsyncHandler(String name) {
        super(name);
    }
    ...
    public void asyncHandle(ParamWrapper paramWrapper, AsyncResult asyncResult, AsyncScheduler asyncScheduler) {
        MysqlPool.mysql.getConnection((res)->{
            if(res.succeeded()){
                ...
                connection.preparedQuery("insert into config_subset_template_structure (subset_template_id, level_key, create_user, update_user, create_time, update_time) value (?,?,?,?,?,?)")
                        .execute(Tuple.of(appData.getSudsetTemplateId(),appData.getKeyLevel(),appData.getCreateUser(),appData.getUpdateUser(),System.currentTimeMillis(),System.currentTimeMillis()),(result)->{
                            if(result.succeeded()){
