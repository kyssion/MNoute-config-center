...
@MintFlowHandler
public class InsertConfigInformationAsyncHandler extends AsyncSampleFnHandler {
    ...
    public InsertConfigInformationAsyncHandler(String name) {
        super(name);
    }
    ...
    public void asyncHandle(ParamWrapper paramWrapper, AsyncResult asyncResult, AsyncScheduler asyncScheduler) {
        MysqlPool.mysql.getConnection((res)->{
            if(res.succeeded()){
                ...
                if(ObjectUtils.isNullOrEmpty(appData.getAppId(),appData.getConfigId(),appData.getGroupId())){
