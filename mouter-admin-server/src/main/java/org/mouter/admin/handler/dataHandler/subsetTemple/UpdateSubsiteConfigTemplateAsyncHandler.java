...
public class UpdateSubsiteConfigTemplateAsyncHandler extends AsyncSampleFnHandler {
    ...
    public UpdateSubsiteConfigTemplateAsyncHandler(String name) {
        super(name);
    }
    ...
    public void asyncHandle(ParamWrapper paramWrapper, AsyncResult asyncResult, AsyncScheduler asyncScheduler) {
        MysqlPool.mysql.getConnection((res)->{
            if(res.succeeded()){
                ...
                connection.preparedQuery("select * from config_subset_template where group_id=? and app_id=? and subset_id=?")
                        .execute(Tuple.of(appData.getGroupId(),appData.getAppId(),appData.getSudsetId()),(result)->{
                            if(result.succeeded()){
                                ...
                                connection.preparedQuery("update config_subset_template set subset_name=?,update_time=?,update_user=? where app_id=? and group_id=? and subset_id=?")
