...
@MintFlowHandler
public class DeleteGroupAsyncHandler extends AsyncSampleFnHandler {

    public static String SQL_DATA_KEY = "data.delete.group";
    public static String SQL_DATA_RESULT_KEY = "data.delete.application.group";

    public DeleteGroupAsyncHandler(String name) {
        super(name);
    }
    ...
    public void asyncHandle(ParamWrapper paramWrapper, AsyncResult asyncResult, AsyncScheduler asyncScheduler) {
        MysqlPool.mysql.getConnection((res)->{
            if(res.succeeded()){
                ...
                connection.preparedQuery("delete from group_information where group_id = ?")
